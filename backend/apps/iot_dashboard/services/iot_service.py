...
from ..models import AlertRule, IoTAlert, IoTDevice, SensorReading
...
class IoTService:
    ...
    def record_reading(device_id, value, metadata=None):
        ...
        reading = SensorReading.objects.create(
            device=device, value=value, unit=device.sensor_type.unit, metadata=metadata or {}
        )
    ...
    def get_school_dashboard(school_id):
        ...
        alerts = IoTAlert.objects.filter(device__school_id=school_id, is_acknowledged=False).count()
    ...
    def get_energy_dashboard(school_id):
        devices = IoTDevice.objects.filter(school_id=school_id, sensor_type__code="ENERGY")
        total_consumption = SensorReading.objects.filter(device__in=devices).aggregate(total=Avg("value"))["total"] or 0
