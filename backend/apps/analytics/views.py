...
class DashboardStatsViewSet(viewsets.ViewSet):
    ...
    def school_stats(self, request):
        ...
        schools = (
            ...
            .annotate(student_count=Count("students"), staff_count=Count("staff"))
    ...
    def attendance_stats(self, request):
        ...
        stats = StudentAttendance.objects.filter(date__gte=week_ago).values("status").annotate(count=Count("id"))
    ...
    def financial_stats(self, request):
        ...
        total_collected = Payment.objects.filter(is_confirmed=True).aggregate(total=Sum("amount"))["total"] or 0

        total_due = (
            StudentFee.objects.filter(status__in=["PENDING", "PARTIAL"]).aggregate(total=Sum("balance"))["total"] or 0
        )
        ...
        return Response(
            {
                ...
                "total_due": total_due,
                "collection_rate": (
                    (total_collected / (total_collected + total_due) * 100) if (total_collected + total_due) > 0 else 0
                ),
            }
        )
    ...
    def recent_activity(self, request):
        ...
        recent_files = (
            ...
            .values(
                "file__file_number",
                "file__title",
                "from_holder__first_name",
                "to_holder__first_name",
                "action",
                "movement_date",
            )
        )
        ...
        recent_tasks = (
            Task.objects.select_related("workflow_instance", "assigned_to")
            .filter(status__in=["PENDING", "IN_PROGRESS"])
            .order_by("-created_at")[:10]
            .values("workflow_instance__reference_number", "assigned_to__first_name", "status", "due_date")
        )

        return Response({"recent_files": list(recent_files), "recent_tasks": list(recent_tasks)})
    ...
    def user_stats(self, request):
        ...
        by_role = User.objects.filter(is_active=True).values("role").annotate(count=Count("id")).order_by("-count")

        recent_logins = User.objects.filter(last_login__gte=timezone.now() - timedelta(hours=24)).count()
    ...
    def students_by_lga(self, request):
        ...
        stats = (
            Student.objects.filter(status="ACTIVE").values("school__lga").annotate(count=Count("id")).order_by("-count")
        )
    ...
    def staff_by_role(self, request):
        ...
        stats = Staff.objects.filter(is_active=True).values("category").annotate(count=Count("id")).order_by("-count")
    ...
    def overdue_tasks(self, request):
        ...
        overdue = (
            Task.objects.filter(status__in=["PENDING", "IN_PROGRESS"], due_date__lt=timezone.now())
            .select_related("workflow_instance", "assigned_to")
            .order_by("due_date")[:10]
            .values(
                "id",
                "workflow_instance__reference_number",
    ...
    def hr_dashboard(self, request):
        from apps.staff.models import Staff, StaffLeave

        total_staff = Staff.objects.filter(is_active=True).count()
        by_category = (
            Staff.objects.filter(is_active=True).values("category").annotate(count=Count("id")).order_by("-count")
        )
        by_designation = (
            Staff.objects.filter(is_active=True).values("designation").annotate(count=Count("id")).order_by("-count")
        )
        by_employment_type = (
        ...
        new_hires_30d = Staff.objects.filter(date_joined__gte=timezone.now().date() - timedelta(days=30)).count()
        ...
        recent_leaves = list(
            StaffLeave.objects.select_related("staff", "staff__user")
    ...
    def finance_dashboard(self, request):
        ...
        payments_today = (
            Payment.objects.filter(payment_date=timezone.now().date()).aggregate(total=Sum("amount"))["total"] or 0
        )
        ...
        fee_status = list(StudentFee.objects.values("status").annotate(count=Count("id"), total=Sum("amount_due")))
    ...
    def principal_dashboard(self, request):
        ...
        attendance_today = StudentAttendance.objects.filter(student__school=school, date=today)
        ...
        attendance_trend = list(
            StudentAttendance.objects.filter(student__school=school, date__gte=week_ago)
            .values("date")
            .annotate(present=Count("id", filter=Q(status="PRESENT")), total=Count("id"))
            .order_by("date")
        )
        ...
        total_fees_due = (
            StudentFee.objects.filter(student__school=school, status__in=["PENDING", "PARTIAL"]).aggregate(
                total=Sum("balance")
            )["total"]
            or 0
        )
        total_fees_collected = (
            Payment.objects.filter(student_fee__student__school=school, is_confirmed=True).aggregate(
                total=Sum("amount")
            )["total"]
            or 0
        )
        ...
        pending_tasks = (
            Task.objects.filter(assigned_to__staff_profile__school=school, status__in=["PENDING", "IN_PROGRESS"])
            .select_related("workflow_instance")
            .order_by("due_date")[:5]
            .values("id", "workflow_instance__reference_number", "status", "due_date")
        )
    ...
    def teacher_dashboard(self, request):
        ...
        from apps.attendance.models import StaffAttendance, StudentAttendance
        from apps.staff.models import Staff
        from apps.students.models import Student
        ...
        attendance_today = StudentAttendance.objects.filter(student__school=staff_profile.school, date=today)
        ...
        attendance_trend = list(
            StudentAttendance.objects.filter(student__school=staff_profile.school, date__gte=week_ago)
            .values("date")
            .annotate(
    ...
    def registry_dashboard(self, request):
        ...
        files_by_status = list(File.objects.values("status").annotate(count=Count("id")).order_by("status"))
    ...
    def parent_dashboard(self, request):
        ...
        user = request.user
        children = (
            Student.objects.filter(parents__user=user, status="ACTIVE")
            .select_related("school", "class_name")
            .distinct()
        )
