...
class StudentAttendance(models.Model):
    ...
    recorded_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, related_name="student_attendance_records"
    )
...
class StaffAttendance(models.Model):
    ...
    remark = models.CharField(max_length=200, blank=True)
    recorded_by = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.SET_NULL, null=True, related_name="staff_attendance_records"
    )
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    def calculate_rate(self):
        if self.total_school_days > 0 and self.school.current_enrollment > 0:
            self.attendance_rate = (
                self.total_present / (self.total_school_days * self.school.current_enrollment)
            ) * 100
            self.save()
