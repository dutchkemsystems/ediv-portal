...
class ReportModelTest(TestCase):
    def setUp(self):
        ...
        self.report = Report.objects.create(title="Monthly Report", report_type="ACADEMIC", generated_by=self.user)
...
class DashboardModelTest(TestCase):
    def setUp(self):
        ...
        self.dashboard = Dashboard.objects.create(name="Main Dashboard", owner=self.user, is_default=True)
...
class WidgetModelTest(TestCase):
    def setUp(self):
        ...
        self.dashboard = Dashboard.objects.create(name="Test", owner=self.user)
        self.widget = Widget.objects.create(
            dashboard=self.dashboard, title="Stats", widget_type="STAT_CARD", data_source="analytics.overview"
        )
...
class ReportAPITest(APITestCase):
    def setUp(self):
        ...
        self.report = Report.objects.create(title="Test Report", report_type="FINANCIAL", generated_by=self.user)
    ...
    def test_create_report(self):
        response = self.client.post(
            "/api/reports/reports/", {"title": "New Report", "report_type": "ATTENDANCE", "generated_by": self.user.id}
        )
