...
from apps.users.models import Privilege, RolePrivilege
    ...
    def handle(self, *args, **options):
        ...
        for role, description in ROLE_DEFINITIONS.items():
            if dry_run:
                ...
                obj, created = RolePrivilege.objects.get_or_create(role=role, defaults={"description": description})
                if created:
                    templates_created += 1
        ...
        self.stdout.write(
            self.style.SUCCESS(f'Privileges: {privileges_created} {"would be " if dry_run else ""}created')
        )
