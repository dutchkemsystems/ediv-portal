...
class MessageModelTest(TestCase):
    def setUp(self):
        ...
        self.receiver = User.objects.create_user(
            email="receiver@ediv.gov.ng", password="TestPass123!@#", first_name="Receiver", last_name="User", role="TCH"
        )
...
class NotificationModelTest(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            email="user@ediv.gov.ng", password="TestPass123!@#", first_name="Test", last_name="User", role="TCH"
        )
        self.notification = UserNotification.objects.create(
            user=self.user, title="Test Notification", message="This is a test notification", notification_type="INFO"
        )
