...
class InspectionModelTest(TestCase):
    def setUp(self):
        ...
        self.inspector = User.objects.create_user(
            email="inspector@test.com", password="TestPass123!", first_name="Inspector", last_name="One", role="QA"
        )
        self.inspection = Inspection.objects.create(
            school=self.school,
            title="Q1 Routine Inspection",
            inspection_type="ROUTINE",
            scheduled_date="2026-03-01",
            lead_inspector=self.inspector,
            objectives="Check facilities",
        )
...
class InspectionChecklistModelTest(TestCase):
    def setUp(self):
        ...
        self.inspection = Inspection.objects.create(
            ...
            inspection_type="ROUTINE",
            scheduled_date="2026-03-01",
            lead_inspector=inspector,
            objectives="Check",
        )
        self.checklist = InspectionChecklist.objects.create(
            inspection=self.inspection,
            category="Infrastructure",
            item="Classroom condition",
            description="Check walls and roof",
        )
...
class InspectionAPITest(APITestCase):
    ...
    def test_create_inspection(self):
        inspector = User.objects.create_user(
            email="insp@test.com", password="TestPass123!", first_name="Insp", last_name="User", role="QA"
        )
