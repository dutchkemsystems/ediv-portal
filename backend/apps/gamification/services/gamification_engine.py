...
class GamificationEngine:
    ...
    def award_points(user, action_type, description="", reference_id="", reference_model="", awarded_by=None):
        ...
        if category.daily_limit > 0:
            ...
            today_count = PointTransaction.objects.filter(user=user, category=category, created_at__date=today).count()
        ...
        for badge in available_badges:
            if user_points.total_points >= badge.points_required:
                UserBadge.objects.create(
                    user=user, badge=badge, reason=f"Achieved {badge.tier} tier with {user_points.total_points} points"
                )
    ...
    def get_user_stats(user):
        ...
        category_breakdown = transactions.values("category__name").annotate(total=Sum("points")).order_by("-total")
        ...
        return {
            ...
            "badges": [{"name": b.badge.name, "icon": b.badge.icon, "tier": b.badge.tier} for b in badges],
            "recent_transactions": [
                {
    ...
    def initialize_default_categories():
        ...
        for action, name, points, desc in categories:
            PointCategory.objects.update_or_create(
                action_type=action, defaults={"name": name, "points": points, "description": desc}
            )
    ...
    def initialize_default_badges():
        ...
        for name, desc, icon, tier, points in badges:
            Badge.objects.update_or_create(
                name=name, defaults={"description": desc, "icon": icon, "tier": tier, "points_required": points}
            )
