...
class DropoutPredictionEngine:
    ...
    def analyze_student(cls, student):
        ...
        risk_score = sum(scores[k] * cls.WEIGHTS[k] for k in scores)
    ...
    def _attendance_risk(cls, student):
        ...
        try:
            ...
            recent = StudentAttendance.objects.filter(student=student, date__gte=ninety_days_ago)
    ...
    def _academic_risk(cls, student):
        ...
        try:
            ...
            results = ExamResult.objects.filter(student=student).order_by("-exam__exam_date")[:10]
    ...
    def _discipline_risk(cls, student):
        ...
        try:
            ...
            incidents = DisciplinaryIncident.objects.filter(student=student, incident_date__gte=year_start)
            count = incidents.count()
            serious = incidents.filter(severity__in=["HIGH", "CRITICAL"]).count()
    ...
    def _financial_risk(cls, student):
        ...
        try:
            ...
            overdue = fees.filter(due_date__lt=datetime.date.today(), amount_paid__lt=F("amount_due")).count()
    ...
    def _engagement_risk(cls, student):
        ...
        try:
            ...
            activities = ActivityParticipant.objects.filter(student=student, joined_date__gte=term_start).count()
