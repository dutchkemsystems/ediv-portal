...
from apps.predictive_analytics.models import EarlyWarningAlert, RiskTrend, StudentRiskProfile
    ...
    def handle(self, *args, **options):
        ...
        if options["student_id"]:
            ...
            profile, _ = StudentRiskProfile.objects.update_or_create(
                student=student, defaults={**analysis, "last_analyzed": timezone.now()}
            )
            ...
            self.stdout.write(
                self.style.SUCCESS(f'{student} - Risk: {analysis["risk_level"]} ({analysis["risk_score"]:.1f}%)')
            )
            return
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Analysis complete: {saved} students analyzed, {alerts_created} new alerts created.")
        )
