...
from .serializers import (
    IncomingMailListSerializer,
    IncomingMailSerializer,
    MailAssignmentSerializer,
    MailCorrespondenceListSerializer,
    MailCorrespondenceMovementSerializer,
    MailCorrespondenceSerializer,
    MailMovementSerializer,
    OutgoingMailListSerializer,
    OutgoingMailMovementSerializer,
    OutgoingMailSerializer,
    SchoolHQCorrespondenceListSerializer,
...
class IncomingMailViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return (
            IncomingMail.objects.select_related("received_by", "department")
            .filter(
    ...
    def classify_mail(self, request, pk=None):
        ...
        if classification not in valid_classifications:
            return Response(
                {"error": f"Invalid classification. Choose from: {valid_classifications}"},
...
class MailMovementViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        return (
            MailMovement.objects.select_related("mail", "from_person", "to_person")
            .filter(db_models.Q(from_person=user) | db_models.Q(to_person=user) | db_models.Q(mail__received_by=user))
            .distinct()
        )
...
class OutgoingMailMovementViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        return (
            OutgoingMailMovement.objects.select_related("outgoing_mail", "from_person", "to_person")
            .filter(
                db_models.Q(from_person=user)
                | db_models.Q(to_person=user)
...
class SchoolHQCorrespondenceMovementViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        return (
            SchoolHQCorrespondenceMovement.objects.select_related("correspondence", "from_person", "to_person")
            .filter(
                db_models.Q(from_person=user)
                | db_models.Q(to_person=user)
                | db_models.Q(correspondence__sender=user)
                | db_models.Q(correspondence__recipient=user)
            )
            .distinct()
        )
...
class MailCorrespondenceMovementViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        user = self.request.user
        if user.role in ("SYSADMIN", "TG_PS"):
            return MailCorrespondenceMovement.objects.select_related("correspondence", "from_person", "to_person").all()
        return (
            MailCorrespondenceMovement.objects.select_related("correspondence", "from_person", "to_person")
            .filter(
                db_models.Q(from_person=user)
                | db_models.Q(to_person=user)
                | db_models.Q(correspondence__sender=user)
...
class OutgoingMailViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return (
            OutgoingMail.objects.select_related("created_by", "department")
            .filter(db_models.Q(created_by=user) | db_models.Q(approvals__approver=user))
            .distinct()
        )
    ...
    def deliver_mail(self, request, pk=None):
        ...
        if mail_obj.status != "DISPATCHED":
            return Response(
                {"error": "Only DISPATCHED mails can be marked as delivered."}, status=status.HTTP_400_BAD_REQUEST
            )
...
class SchoolHQCorrespondenceViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return (
            SchoolHQCorrespondence.objects.select_related("sender", "recipient", "school", "department")
            .filter(db_models.Q(sender=user) | db_models.Q(recipient=user))
            .distinct()
        )
    ...
    def submit_correspondence(self, request, pk=None):
        ...
        if correspondence.status != "DRAFT":
            return Response(
                {"error": "Only DRAFT correspondences can be submitted."}, status=status.HTTP_400_BAD_REQUEST
            )
...
class MailCorrespondenceViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return (
            MailCorrespondence.objects.select_related("sender", "recipient", "department", "school")
            .filter(db_models.Q(sender=user) | db_models.Q(recipient=user))
            .distinct()
        )
