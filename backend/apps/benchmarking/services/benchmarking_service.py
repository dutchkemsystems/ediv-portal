...
class BenchmarkingService:
    ...
    def calculate_school_metrics(school_id, period=None):
        ...
        from apps.students.models import AcademicRecord, Student

        school_students = Student.objects.filter(school_id=school_id)

        metrics = {}

        avg_score = AcademicRecord.objects.filter(student__school_id=school_id).aggregate(avg=Avg("score"))["avg"] or 0
        ...
        total_fees = (
            FeeBalance.objects.filter(student__school_id=school_id).aggregate(total=Avg("amount_owed"))["total"] or 0
        )
        paid_fees = (
            FeeBalance.objects.filter(student__school_id=school_id).aggregate(total=Avg("amount_paid"))["total"] or 0
        )
        metrics["FEE_COLLECTION_RATE"] = (paid_fees / total_fees * 100) if total_fees > 0 else 0

        metrics["STUDENT_TEACHER_RATIO"] = school_students.count() / 25  # Placeholder
        ...
        for code, value in metrics.items():
            ...
            if metric:
                SchoolBenchmark.objects.update_or_create(
                    school_id=school_id, metric=metric, period=period, defaults={"value": value}
                )
    ...
    def compare_schools(school_a_id, school_b_id, metric_code=None):
        ...
        for metric in metrics:
            bench_a = (
                SchoolBenchmark.objects.filter(school_id=school_a_id, metric=metric).order_by("-calculated_at").first()
            )
            bench_b = (
                SchoolBenchmark.objects.filter(school_id=school_b_id, metric=metric).order_by("-calculated_at").first()
            )
    ...
    def get_district_rankings(metric_code, period=None):
        ...
        benchmarks = SchoolBenchmark.objects.filter(metric=metric).select_related("school")

        if period:
            benchmarks = benchmarks.filter(period=period)

        rankings = []
        sorted_benchmarks = sorted(benchmarks, key=lambda x: x.value, reverse=metric.higher_is_better)
    ...
    def get_school_performance_summary(school_id):
        benchmarks = (
            SchoolBenchmark.objects.filter(school_id=school_id).select_related("metric").order_by("-calculated_at")
        )
    ...
    def get_top_performing_schools(metric_code, limit=10):
        ...
        benchmarks = (
            SchoolBenchmark.objects.filter(metric=metric)
            .select_related("school")
            .order_by("-value" if metric.higher_is_better else "value")[:limit]
        )
