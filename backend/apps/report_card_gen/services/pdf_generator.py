...
class ReportCardGenerator:
    ...
    def generate_report_card(student, academic_session, term, generated_by=None):
        ...
        try:
            ...
            records = AcademicRecord.objects.filter(student=student, academic_session=academic_session, term=term)
            ...
            all_students_avg = (
                AcademicRecord.objects.filter(academic_session=academic_session, term=term)
                .values("student")
    ...
    def _generate_pdf(report_card, records):
        ...
        header_style = ParagraphStyle(
            "Header", parent=styles["Heading1"], alignment=TA_CENTER, fontSize=16, spaceAfter=5
        )
        subheader_style = ParagraphStyle(
            "SubHeader", parent=styles["Heading2"], alignment=TA_CENTER, fontSize=12, spaceAfter=10
        )
        ...
        for record in records:
            table_data.append([record.subject, f"{record.score}%", record.grade, record.remark or ""])

        table_data.append(["Total", f"{report_card.total_score}", "", ""])
        table_data.append(["Average", f"{report_card.average_score}%", "", report_card.remark])
        ...
        if report_card.teacher_remark:
            elements.append(Paragraph("<b>Class Teacher Remark:</b>", styles["Normal"]))
    ...
    def generate_batch(school_id, academic_session, term, class_level=None):
        ...
        for student in students:
            try:
                report = ReportCardGenerator.generate_report_card(student, academic_session, term)
                results.append(
                    {
