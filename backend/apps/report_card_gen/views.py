...
class GeneratedReportCardViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return GeneratedReportCard.objects.filter(student__user=self.request.user)
    ...
    def generate_batch(self, request):
        ...
        if not school_id or not academic_session or not term:
            return Response(
                {"error": "school_id, academic_session, and term required"}, status=status.HTTP_400_BAD_REQUEST
            )

        results = ReportCardGenerator.generate_batch(school_id, academic_session, term, class_level)
        return Response(
            {
    ...
    def share_report(self, request, pk=None):
        ...
        ReportCardShareLog.objects.create(
            report_card=report,
            channel=channel,
            recipient=recipient,
            ...
            return Response(
                {
                    ...
                    "share_url": share_url,
                    "whatsapp_text": f"Report Card for {report.student.user.get_full_name()} is ready. Download: {share_url}",  # noqa: E501
                }
            )
...
class ReportCardShareLogViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        return ReportCardShareLog.objects.filter(report_card__student__user=self.request.user)
