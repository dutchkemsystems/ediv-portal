...
class DocumentModelTest(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            email="user@ediv.gov.ng", password="TestPass123!@#", first_name="Test", last_name="User", role="REG"
        )
...
class ApproveDocumentTest(APITestCase):
    ...
    def setUp(self):
        self.reg_user = User.objects.create_user(
            email="reg@ediv.gov.ng", password="RegPass123!@#", first_name="Reg", last_name="Officer", role="REG"
        )
        self.tg = User.objects.create_user(
            email="tg@ediv.gov.ng", password="TgPass123!@#", first_name="Tutor", last_name="General", role="TG_PS"
        )
...
class RejectDocumentTest(APITestCase):
    ...
    def test_reg_officer_can_reject(self):
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {self.reg_token.access_token}")
        response = self.client.post(
            f"/api/registry/documents/{self.doc.id}/reject/",
            {
...
class FilingViewSetTest(APITestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(
            email="filer@ediv.gov.ng", password="FilerPass123!@#", first_name="Filer", last_name="User", role="REG"
        )
        self.doc = Document.objects.create(
