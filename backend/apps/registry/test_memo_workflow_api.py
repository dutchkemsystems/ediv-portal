...
from apps.registry.models import (
    Document, MemoWorkflow, MemoApproval, MemoCirculation
)
...
class MemoWorkflowAPITest(APITestCase):
    ...
    def test_memo_full_flow(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(MemoCirculation.objects.filter(
            memo_workflow=self.memo, recipient=self.recipient, status='SENT'
        ).count(), 1)
