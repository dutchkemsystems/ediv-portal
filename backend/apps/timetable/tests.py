...
class PeriodModelTest(TestCase):
    def setUp(self):
        ...
        self.period = Period.objects.create(
            school=self.school, name="Period 1", start_time="08:00", end_time="08:40", period_number=1
        )
...
class TimetableModelTest(TestCase):
    def setUp(self):
        from apps.academics.models import Class, ClassLevel

        self.school = School.objects.create(
            name="Test School", code="TST", school_type="SENIOR", lga="APAPA", address="123 Street"
        )
        self.class_obj = Class.objects.create(
            school=self.school, name="JSS 1A", level=ClassLevel.JSS1, academic_year="2025/2026"
        )
        self.timetable = Timetable.objects.create(
            school=self.school, class_obj=self.class_obj, academic_year="2025/2026", term="FIRST"
        )
