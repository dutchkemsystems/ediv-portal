...
class AlumniMemberModelTest(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            email="alumni@test.com", password="TestPass123!", first_name="Alumni", last_name="Member", role="STD"
        )
        self.school = School.objects.create(
            name="Test School", code="TST", school_type="SENIOR", lga="APAPA", address="123 Street"
        )
        self.alumni = AlumniMember.objects.create(
            user=self.user, school=self.school, graduation_year=2020, current_occupation="Engineer", company="Tech Corp"
        )
...
class AlumniDonationModelTest(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            email="donor@test.com", password="TestPass123!", first_name="Donor", last_name="User", role="STD"
        )
        school = School.objects.create(
            name="Test School", code="TST", school_type="SENIOR", lga="APAPA", address="123 Street"
        )
        self.alumni = AlumniMember.objects.create(user=self.user, school=school, graduation_year=2020)
