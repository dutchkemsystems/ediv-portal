...
class FileMovementService:
    ...
    INCOMING_WORKFLOW = [
        ...
        {
            "step": 2,
            "location": "TG_PS_OFFICE",
            "role": "REG_OFF",
            "deadline": 4,
            "label": "Clerical Officer Review",
        },
        {
            "step": 3,
            "location": "TG_PS_OFFICE",
        ...
        {
            "step": 7,
            "location": "DESK_OFFICER",
            "role": "TCH",
            "deadline": 48,
            "label": "Desk Officer Processing",
        },
        {
            "step": 8,
            "location": "UNIT",
            "role": "SA_OFF",
            "deadline": 24,
            "label": "Unit Head Completion Review",
        },
        {
            "step": 9,
            "location": "DEPARTMENT",
            "role": "SA_OFF",
            "deadline": 24,
            "label": "Department Head Completion Review",
        },
        {
            "step": 10,
            "location": "TG_PS_OFFICE",
    ...
    def move_file(
        *,
        ...
        movement = FileMovement(
            ...
            from_location=(
                FileMovementService._get_step_info(current_step, direction)["location"]
                if current_step > 0
                else "CREATED"
            ),
        ...
        AuditService.log_action(
            ...
            resource_type="File",
            resource_id=file.id,
            description=f"Moved file {file.file_number} from {from_holder.get_full_name()} to {to_holder.get_full_name()}: {action}",  # noqa: E501
            old_value={"current_holder": from_holder.id, "status": file.status},
            new_value={
                "current_holder": to_holder.id if to_holder else None,
                "action": action,
            },
        )
        ...
        for file_obj in overdue_files:
            try:
                ...
                FileMovementService._add_timeline_entry(
                    ...
                    file_obj.created_by,
                    "ESCALATED",
                    "Auto-escalated: deadline passed",
                )
                file_obj.save(update_fields=["status_timeline"])
                escalated_count += 1
    ...
    def get_file_status(file_id):
        ...
        result = {
            ...
            "assigned_department": (
                ...
                if file_obj.assigned_department
                else None
            ),
            "school": (
                {
