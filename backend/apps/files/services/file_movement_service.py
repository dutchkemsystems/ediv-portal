...
class FileMovementService:
    ...
    def escalate_file(*, file, escalated_by, reason=""):
        ...
        FileMovementService._add_timeline_entry(
            file, file.status, escalated_by, "ESCALATED", reason or f"Escalated from {old_priority} to {new_priority}"
        )
        file.save(
            update_fields=[
                "priority",
    ...
    def check_and_escalate_overdue():
        ...
        for file_obj in overdue_files:
            try:
                ...
                file_obj.save(
                    update_fields=["priority", "escalation_status", "escalation_reason", "escalated_at", "updated_at"]
                )
                ...
                FileMovementService._add_timeline_entry(
                    file_obj, file_obj.status, file_obj.created_by, "ESCALATED", "Auto-escalated: deadline passed"
                )
                file_obj.save(update_fields=["status_timeline"])
                escalated_count += 1
    ...
    def get_user_pending_files(user):
        ...
        return File.objects.filter(
            current_holder=user, status__in=["ACTIVE", "PENDING", "IN_TRANSIT", "UNDER_REVIEW"]
