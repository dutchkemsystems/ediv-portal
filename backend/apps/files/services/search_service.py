...
class SearchService:
    ...
    def search_movements(
        *, file_id=None, from_holder=None, to_holder=None, action=None, date_from=None, date_to=None, limit=50, offset=0
    ) -> dict:
    ...
    def get_search_suggestions(query, limit=10) -> list:
        ...
        files = File.objects.filter(Q(file_number__icontains=query) | Q(title__icontains=query)).values(
            "file_number", "title"
        )[:limit]
