...
class OfflineSyncService:
    ...
    @staticmethod
    def queue_action(*, user, object_id, action_type, data) -> "OfflineQueue":  # noqa: F821
        """Add an action to the offline queue."""
        from apps.files.models import OfflineQueue

        queue_item = OfflineQueue.objects.create(
            object_id=object_id, action_type=action_type, user=user, data=data, status="PENDING"
        )
    ...
    def process_queue(*, user=None, limit=100) -> dict:
        ...
        """
        from apps.files.models import File, OfflineQueue
    ...
    def retry_failed(*, user=None, max_attempts=3) -> dict:
        ...
        qs = OfflineQueue.objects.filter(status="FAILED", attempt_count__lt=max_attempts)
        if user:
            qs = qs.filter(user=user)
    ...
    def clear_completed(*, user=None, older_than_days=30) -> int:
        ...
        qs = OfflineQueue.objects.filter(status="COMPLETED", processed_at__lt=cutoff)
        if user:
            qs = qs.filter(user=user)
