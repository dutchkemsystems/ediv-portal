...
class OfflineSyncServiceQueueTest(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(
            email="offline@ediv.gov.ng", password="TestPass123!@#", first_name="Offline", last_name="User", role="TCH"
        )
...
class OfflineSyncServiceProcessTest(TestCase):
    ...
    def test_process_move_action(self):
        ...
        other_user = User.objects.create_user(
            email="target@ediv.gov.ng", password="Test123!@#", first_name="Target", last_name="User", role="TCH"
        )
        OfflineSyncService.queue_action(
            user=self.user,
            object_id=str(file_obj.id),
