...
INTENT_PATTERNS = {
    ...
    "contact_teacher": {
        "patterns": [
            ...
            "talk to my child teacher",
            "contact my child teacher",
        ],
        "min_role": ["PARENT"],
    },
    "report_issue": {
...
class ChatbotEngine:
    ...
    def _handle_check_fees(user, context):
        ...
        if user.role == "STUDENT":
            ...
            for f in fees:
                status = (
                    "Paid"
                    if f.amount_paid >= f.amount_owed
                    else f"NGN {f.amount_owed - f.amount_paid:,.2f} outstanding"
                )
    ...
    def _handle_contact_teacher(user, context):
        return (
            "To contact a teacher, please use the Communication module in the portal, "
    ...
    def _handle_timetable(user, context):
        return (
            "To view your timetable, go to the **Timetable** section in the portal. "
