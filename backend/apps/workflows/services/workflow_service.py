...
APPROVAL_ACTIONS = {
    'APPROVED', 'APPROVAL', 'FINAL_APPROVAL', 'TG_APPROVAL',
    'DEPT_HEAD_APPROVAL', 'DEPT_APPROVAL', 'PRINCIPAL_REVIEW',
}
...
class WorkflowService:
    ...
    def advance(instance, *, user, decision='APPROVE', comments='') -> dict:
        ...
        if current_task:
            if (current_task.assigned_to != user
                    and not user.is_superuser
                    and user.role != 'SYSADMIN'):
                raise ValueError(
                    f'User {user.email} is not assigned to step {instance.current_step.name}.'
                )
    ...
    def _assign_task(instance, step):
        ...
        if step.assigned_role:
            assignee = User.objects.filter(
                role=step.assigned_role, is_active=True
            ).order_by('id').first()
        if assignee is None:
            assignee = User.objects.filter(
