...
class WorkflowModelTest(TestCase):
    def setUp(self):
        ...
        self.step = WorkflowStep.objects.create(
            workflow=self.workflow, name="Manager Approval", step_type="APPROVAL", order=1, is_required=True
        )
...
class WorkflowAPITest(APITestCase):
    def setUp(self):
        ...
        self.workflow = Workflow.objects.create(
            name="Test Workflow", description="Test workflow", created_by=self.admin, status="ACTIVE"
        )
