...
class WorkflowAPIActionTest(APITestCase):
    ...
    def test_start_action_creates_instance(self):
        response = self.client.post('/api/workflows/instances/start/', {
    ...
    def test_advance_action_moves_to_next_step(self):
        ...
        response = self.client.post(f'/api/workflows/instances/{instance.id}/advance/', {
            'decision': 'APPROVE',
            'comments': 'Looks good',
        })
    ...
    def test_advance_action_completes_workflow(self):
        ...
        self.assertEqual(instance.tasks.filter(status='COMPLETED').count(),
                         instance.workflow.steps.count())
