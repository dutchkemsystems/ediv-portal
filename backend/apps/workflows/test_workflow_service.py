...
class WorkflowServiceInstanceTest(TestCase):
    ...
    def test_advance_walks_through_all_steps(self):
        ...
        for _ in range(step_count - 1):
            ...
            assignee = instance.tasks.filter(
                step=current_step, status='PENDING'
            ).first().assigned_to
        ...
        final_assignee = instance.tasks.filter(
            step=final_step, status='PENDING'
        ).first().assigned_to
        ...
        self.assertEqual(
            instance.tasks.filter(status='COMPLETED').count(), step_count
        )
