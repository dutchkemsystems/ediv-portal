...
class AuditLogModelTest(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            email="auditor@test.com", password="TestPass123!", first_name="Auditor", last_name="One", role="SYSADMIN"
        )
...
class ComplianceRecordModelTest(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            email="user@test.com", password="TestPass123!", first_name="User", last_name="One", role="SYSADMIN"
        )
        self.school = School.objects.create(
            name="Test School", code="TST", school_type="SENIOR", lga="APAPA", address="123 Street"
        )
        self.item = ComplianceItem.objects.create(
            category="FINANCIAL", title="Budget Report", description="Submit annual budget", frequency="Annual"
        )
        self.record = ComplianceRecord.objects.create(
            item=self.item, school=self.school, academic_year="2025/2026", due_date="2026-06-30", created_by=self.user
        )
...
class ViolationModelTest(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            email="reporter@test.com", password="TestPass123!", first_name="Reporter", last_name="One", role="SYSADMIN"
        )
        self.school = School.objects.create(
            name="Test School", code="TST", school_type="SENIOR", lga="APAPA", address="123 Street"
        )
        self.violation = Violation.objects.create(
            title="Late Fee Submission",
            description="Fees submitted past deadline",
            severity="MEDIUM",
            category="FINANCIAL",
            school=self.school,
