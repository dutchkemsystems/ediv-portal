...
from .models import JobPosting, PayrollPeriod, Payslip
...
class JobPostingModelTest(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            email="hr@test.com", password="TestPass123!", first_name="HR", last_name="Head", role="HR"
        )
...
class PayrollPeriodModelTest(TestCase):
    def setUp(self):
        self.period = PayrollPeriod.objects.create(
            name="January 2026", start_date="2026-01-01", end_date="2026-01-31", payment_date="2026-02-05"
        )
...
class PayslipModelTest(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(
            email="staff@test.com", password="TestPass123!", first_name="Staff", last_name="Member", role="TCH"
        )
        school = School.objects.create(
            name="Test School", code="TST", school_type="SENIOR", lga="APAPA", address="123 Street"
        )
        self.staff = Staff.objects.create(
            ...
            bank_account_name="Staff Member",
            date_joined="2020-01-01",
        )
        self.period = PayrollPeriod.objects.create(
            name="Jan 2026", start_date="2026-01-01", end_date="2026-01-31", payment_date="2026-02-05"
        )
...
class HRAPITest(APITestCase):
    def setUp(self):
        ...
        self.dept = Department.objects.create(name="HR", code="HR", category="CORE")
        self.posting = JobPosting.objects.create(
            title="Teacher", description="Teach", department=self.dept, created_by=self.user, closing_date="2026-12-31"
        )
