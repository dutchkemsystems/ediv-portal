...
class NotificationConsumer(AsyncWebsocketConsumer):
    ...
    def mark_notification_read(self, notification_id):
        ...
        UserNotification.objects.filter(id=notification_id, user=self.user).update(is_read=True)

    @database_sync_to_async
    def mark_all_read(self):
        from apps.notifications.models import UserNotification

        UserNotification.objects.filter(user=self.user, is_read=False).update(is_read=True)
...
class DashboardConsumer(AsyncWebsocketConsumer):
    ...
    def get_dashboard_stats(self):
        ...
        elif self.user.role in ("HR", "FIN", "AUDIT", "QA", "CC", "EMIS", "PLAN", "PROC", "PA", "SA", "FRENCH", "REG"):
            stats["held_files"] = File.objects.filter(current_holder=self.user).count()
